...
+ [修复] 恢复火山方舟配置导出和同源代理 URL 处理，避免前端入口加载失败出现空白页面。
+ [修复] 本地启动脚本统一打开 `http://localhost:3000/`，避免与 `127.0.0.1` 产生隔离的浏览器项目和配置数据。
+ [修复] Canvas Agent 配置损坏时明确终止启动，不再生成临时 token 导致网页显示误导性的连接失败。
+ [修复] Canvas Agent 配置改为临时文件原子替换，避免写入中断造成 token 和项目注册表损坏。
+ [修复] 恢复画布普通滚轮平移、Shift 横向平移和 Ctrl 缩放，并允许鼠标经过普通文本节点时继续平移。
...
+ [调整] 移除影视解说工作台、媒资开发代理及相关产品文档。
+ [优化] Agent 技能面板默认展示当前项目 Skill，并将项目与全局 Skill 分组筛选。

+ [修复] 视频生成配置新增声音开关，画布历史视频节点默认迁移为开启声音生成。
+ [修复] Canvas Agent 结构化询问改为可长时间挂起、重连恢复并幂等结束，避免澄清卡片超时或提交后持续转圈。
+ [新增] Canvas Agent 对话支持修改最后一条输入，并在同一 Codex 会话中重新执行。
+ [新增] Canvas Agent 支持在当前浏览器页面选择默认或独立本地项目工作区，并按目录隔离会话与项目级 Skill。
+ [修复] Canvas Agent 文件夹选择改为 Windows 原生选择窗口，支持取消并避免请求卡在加载状态。
+ [优化] Canvas Agent 选择项目文件夹后自动使用文件夹名称创建项目，无需重复填写项目名称。
+ [修复] Canvas Agent 切换项目遇到失效历史线程时自动创建可用草稿，并在短暂断线后保留 SSE 自动重连。
+ [修复] Canvas Agent 恢复项目历史时改用线程工作目录校验，失效的 `activeThreadId` 会自动清理并创建新对话。
+ [优化] Canvas Agent 按工作目录缓存已完成的 MCP 服务清单，避免历史恢复或项目切换重复预热全部 MCP。
+ [调整] Agent 连接成功后不再弹出遮挡顶部导航的全局消息。
+ [调整] 顶部 GitHub 图标改为跳转到个人 fork 仓库。
+ [优化] Canvas Agent 遇到可修复的生成参数错误时原地更新原配置节点并重试，避免失败一次就新增完整生成链路。
+ [优化] 画布生成节点改用任务摘要与类型命名，去除提示词截断和引用占位符，并保留用户手动标题。
+ [修复] 恢复视频配置依赖的 `boolConfig` 导出，避免入口加载失败导致页面白屏。
+ [修复] 火山方舟视频改用异步任务接口并通过同源代理转发，避免浏览器跨域拦截。

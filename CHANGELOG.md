...
+ [调整] 画布滚轮默认平移，按住 Shift 水平平移、按住 Control 缩放。
+ [修复] 滚轮平移经过普通文本节点时不再被节点内容截断。
+ [新增] 提供静默启动和停止本地 Vite、Canvas Agent 的 Windows 批处理脚本。

## v0.15.2 - 2026-08-10

+ [新增] 官方火山方舟模型列表通过同源代理自动拉取，兼容 Vite、Docker/Render 与 Vercel 部署。
